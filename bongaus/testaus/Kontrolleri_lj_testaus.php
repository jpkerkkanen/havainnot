 ...
 * @author J-P
 */
class Kontrolleri_lj_testaus extends Testiapu_bongaus {
    ...
    public function  __construct($tietokantaolio, $parametriolio) {
        ...
        $kayttajan_id = 1;
        $kokoelmanimi = ""; // Koskee kuvia vain.
        $parametriolio = new Parametrit($kokoelmanimi,
                                        $kayttajan_id,
                                        $tietokantaolio);

        $PK = Pikakommentti::$PARAMETRI_EI_KAYTOSSA;
    ...
    /**
     * KESKEN! Oikeastaan vain alkutoimenpiteet suoritettu.
     */
    public function suorita_testit_ja_siivoa(){
        $this->lisaa_testikommentti("Haetaan tietokannasta oikean henkilon id,
                sita tarvitaan pikakommentin viite-eheys-juttuihin", false);
        
        $this->id_testihenkilo1 = $this->hae_henkilon_id();

        $this->lisaa_testikommentti("Luodaan ja tallennetaan
                    tietokantaan kolme uutta pikakommenttia.",false);
        ...
        if($tallennetun_id != Pikakommentti::$MUUTTUJAA_EI_MAARITELTY){
            $this->lisaa_pikakommentti(new Pikakommentti($this->tietokantaolio,
                                                        $tallennetun_id));
        ...
        if($palaute==3){
            $this->lisaa_testikommentti("Poisto onnistui! $palaute
                                        pikakommenttia kolmesta poistettu!",false);
        }
        else{
            $this->lisaa_testikommentti("Virhe poistossa! Vain $palaute
                                        pikakommenttia poistettu, vaikka
                                        kolme piti poistaa!!",true);
        }
        //======================================================================
        $uusi = "<h3>Kontrolleritestit suoritettu!</h3>";
        $this->lisaa_testikommentti($uusi, false);
    ...
    public function siivoa_jaljet(){
        ...
        $this->lisaa_testikommentti(
                    "Tietokannasta poistettu ".$lkm." henkiloa.",false);
        ...
        if($lkm == 0){
            $this->lisaa_testikommentti(
                    "Tietokannasta ei loytynyt testipikakommenttia, koska se
                        ...
                        poiston yhteydessa (cascade - delete -juttu). OIKEIN!",false);
        }
        else{
            $lisaa_virheilm = true;
            $uusi = "Virhe: pikakommentit eivät olleet hävinneet, vaikka näin
                pitäisi käydä, kun henkilö hävitetään.";
            $this->lisaa_testikommentti($uusi, $lisaa_virheilm);
        }
    }
